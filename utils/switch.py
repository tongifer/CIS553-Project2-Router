...
class SwitchConnection(object):
    ...
    def GetDigest(self, digest_config, dry_run=False):
        if digest_config.digest_id not in self.digests_configured:
            self.digests_configured.add(digest_config.digest_id)
            self.ConfigureDigest(digest_config, dry_run)
        ...
        assert digest_config.digest_id == response.digest.digest_id \
               and "Multiple unique digests are not supported yet!"
