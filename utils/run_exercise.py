...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        for link in host_links:
            ...
            sw_mac = '00:00:00:%02x:%02x:00' % (sw_num, host_num)
        ...
        for link in switch_links:
            node1_num = int(link['node1'][1:])
            node2_num = int(link['node2'][1:])
            node1_mac = '00:00:00:%02x:%02x:00' % (node1_num, node2_num)
            node2_mac = '00:00:00:%02x:%02x:00' % (node2_num, node1_num)

            self.addLink(link['node1'], link['node2'],
                        delay=link['latency'], bw=link['bandwidth'],
                        addr1=node1_mac, addr2=node2_mac)
